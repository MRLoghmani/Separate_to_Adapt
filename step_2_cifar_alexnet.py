...
discriminator_t.load_state_dict(torch.load('discriminator_a_cifar_alexnet.pkl'))
...
cls = CLS(feature_extractor.output_num(), 6, bottle_neck_dim=256)
...
while k <1500:
    for i, (batch_source, batch_target) in enumerate(zip(source_loader, target_loader)):
        ...
        try:
            ...
            ce_ep = CrossEntropyLoss(Variable(torch.from_numpy(np.concatenate((np.zeros((2,5)), np.ones((2,1))), axis = -1).astype('float32'))).cuda(),predict_prob_otherep)
        except:
            continue
