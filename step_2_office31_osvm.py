...
def transform(data, label, is_train):
    ...
    data = np.asarray(data, np.float32) / 255.0
    return data, label
ds1 = FileListDataset('/mnt/datasets/office-31/'+target_ds+'/images/os_split_score.txt', '/mnt/datasets/office-31/'+target_ds+'/images/', transform=transform, skip_pred=skip, is_train=True, imsize=256)
...
acc_os_star = sum([cm[i][i] for i in range(num_known_classes)]) / num_known_classes
...
with open(source_ds+'_'+target_ds+'.txt', 'w+') as f:
